...
class EarlyStopping:
    ...
    def __call__(self, epoch, scores, pvalues, model, ckpt_name = 'checkpoint.pt'):
        ...
        if self.best_score is None:
            ...
            self.counter += 1
            if self.type == 'min':
                self.logger.info(f'best metric: {-self.best_score}, current metric: {-score}')
            elif self.type == 'max':
                self.logger.info(f'best metric: {self.best_score}, current metric: {score}')
            else:
    ...
    def save_checkpoint(self, score, model, ckpt_name):
        ...
        if self.type == 'max':
            self.logger.info(f'metric increased ({self.best_score:.6f} --> {score:.6f}).  Saving model ...')
        elif self.type == 'min':
            self.logger.info(f'metric decreased ({-self.best_score:.6f} --> {-score:.6f}).  Saving model ...')
        else:
            raise NotImplementedError
